...
let rec string_of_sexpr = function
  ...
  | SId(s) -> s
  | SBinop(e1, o, e2) ->
    S  string_of_sexpr e1 ^ " " ^ string_of_op o ^ " " ^ string_of_sexpr e2
  ...
  | SCall(f, el) ->
    S  f ^ "(" ^ String.concat ", " (List.map string_of_sexpr el) ^ ")"
...
let rec string_of_sstmt = function
  ...
  | SIf(e, s1, s2) ->  "if (" ^ string_of_sexpr e ^ ")\n" ^
    S  string_of_sstmt s1 ^ "else\n" ^ string_of_sstmt s2
  | SFor(e1, e2, e3) ->
    S  "for " ^ string_of_sexpr e1  ^ " in " ^ string_of_sexpr e2  ^ string_of_sstmt e3
...
let string_of_sfdecl fdecl =
  ...
  sfdecl.sfname ^ "(" ^ String.concat ", " (List.map snd fdecl.sformals) ^
  ")\n{\n" ^
